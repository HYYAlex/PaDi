...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.data_path.endswith(".npz"):
            ...
            df_raw.columns = [f"var_{i}" for i in range(df_raw.shape[1])]


            df_raw.insert(0, "date", pd.date_range(
                start="2018-09-01", periods=len(df_raw), freq="5min"
            ))
            ...
            df_raw = pd.read_csv(path, header=None, sep=",")
            df_raw.columns = [f"var_{i}" for i in range(df_raw.shape[1])]
            df_raw.insert(0, "date", pd.date_range(
                start="2006-01-01", periods=len(df_raw), freq="10min"
            ))
            if self.target not in df_raw.columns:
                self.target = df_raw.columns[-1]
            ...
            try:
                ...
                try:
                    ...
                    df_raw = pd.read_csv(path, encoding="gb18030", errors="replace")
        ...
        if "date" in cols:
            ...
            df_raw.insert(0, "date", pd.date_range("2018-09-01", periods=len(df_raw), freq="5min"))
